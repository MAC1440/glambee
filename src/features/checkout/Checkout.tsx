  ...
  const handleDiscountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value > subtotal) {
      ...
      toast({
        ...
        description: `Discount cannot be greater than the subtotal of PKR ${subtotal.toFixed(2)}.`,
        variant: "destructive",
      });
    } else {
        ...
        <Card className="flex flex-col">
                ...
                <div className="space-y-2">
                  {cart.map((item, index) => (
                    <div key={index} className="flex justify-between items-start text-sm p-2 rounded-md bg-muted/50">
                      ...
                      <div className="flex items-center gap-4">
                        <span className="font-medium">PKR {(Number(item.service.price) || 0).toFixed(2)}</span>
          ...
          <CardFooter className="grid grid-cols-2 gap-2 mt-auto pt-6 bg-muted/50">
            <Button size="lg" className="col-span-2" disabled={cart.length === 0}>
              <CreditCard className="mr-2" /> Pay PKR {total.toFixed(2)}
            </Button>
