...
export default function SchedulePage() {
    ...
    const weeklyAppointments = appointments.filter(
        (apt) => new Date(apt.date) >= today && new Date(apt.date) <= addDays(today, 7)
    );
    const monthlyAppointments = appointments.filter(
        (apt) => new Date(apt.date).getMonth() === today.getMonth() && new Date(apt.date).getFullYear() === today.getFullYear()
    );
        ...
        {appointments.length > 0 ? (
          appointments.map((apt) => (
            <TableRow key={apt.id}>
              ...
              {showDate && <TableCell className="hidden md:table-cell">{apt.date}</TableCell>}
